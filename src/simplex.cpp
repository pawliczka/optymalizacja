 ...
 * 
 * To jest plik zawierający źródła biblioteki implemetującej algorytm
...
Simplex::Simplex( LinProblem& Problem ) :Table(Problem)
{
	Type = Problem.gettype();
	Solution = new LinProblemSolution( Problem.getn() );
}
//Simplex::Simplex( LinProblem* Problem )
//{
    ...
    for( int j = 1 ; j < Table.getnumCols() ; j++ )
        if( Table.getTabElem(0,j) == 0 )
		{
            ...
            for( int i = 1 ; i < Table.getnumRows() ; i++ )
				...
				{
					State.Case = INF_SOLUTIONS_BOUND;
					return;
				}
		}
}
    ...
    for( int i = 1 ; i < Table.getnumCols() ; i++ )
        if( Table.getTabElem(0,i) == 0 )
		{
            ...
            for( int j = 1 ; j < Table.getnumRows() ; j++ )
				...
				{
					State.Case = INF_SOLUTIONS_UNBOUND;
					return;
				}
		}
}

int Simplex::IsPermissible() const
{
    for( int i = 1 ; i < Table.getnumRows() ; i++ )
        if( Table.getTabElem(i,0) < 0 ) return 0;

	return 1;
}

int Simplex::IsOptimal() const
{
    for( int i = 1 ; i < Table.getnumCols() ; i++ )
        if( Table.getTabElem(0,i) < 0 ) return 0;
...
void Simplex::UpdateSolution()
{
    ...
    for( int i = 1 ; i < Table.getnumRows() ; i++ )
        if( Table.getRowIndex(i) <= n )
            Solution->VariableValues[Table.getRowIndex(i)-1]
                = Table.getTabElem(i,0);
    for( int j = 1 ; j < Table.getnumCols() ; j++ )
        if( Table.getColIndex(j) <= n )
            Solution->VariableValues[Table.getColIndex(j)-1]
				= 0;
}
...
int Simplex::ChooseFromBase2Delete( int BaseVarIndex ) const
{
    ...
    for( int i = 1 ; i < Table.getnumRows() ; i++ )
	{
		...
		{
            TempElem = Table.getTabElem(i,0)
                / Table.getTabElem(i,BaseVarIndex);
...
void Simplex::UpdateTable( int k , int r )
{
    ...
    for( int i = 0 ; i < Table.getnumRows() ; i++ )
        for( int j = 0 ; j < Table.getnumCols() ; j++ )
			if( i != r && j != k )
                Table.setTabElem(i,j, Table.getTabElem(i,j)
                -Table.getTabElem(i,k)*Table.getTabElem(r,j)/centralElement);
...
LinProblemSolution* Simplex::Solve()
{
    Table.print();
	State.Status = STATUS_BUSY;
	State.Case = ONE_SOLUTION;
    Solve1Phase();
	CheckNoSolutions();
	...
	if( Type == MIN ) Solution->ObjFuncValue *= -1;
	return Solution;
}

LinProblemSolution* Simplex::Solve1Phase()
{
	int Step = 0;
...
LinProblemSolution* Simplex::Solve2Phase()
{
	int Step = 0;
	while( !IsOptimal() )
	{
